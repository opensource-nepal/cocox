...
#[test]
fn hash_head_succeeds() {
    // The cocox repo's own HEAD message is a conventional commit
    // (e.g. "feat: ignore autogenerated commit messages (#4)"), so HEAD
    // should lint successfully when run from the project root.
    cocox()
        .arg("--hash")
        .arg("HEAD")
        .assert()
